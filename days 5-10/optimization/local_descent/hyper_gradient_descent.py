...
def hyper_gradient_descent(f, nabla_f, x, alpha=0.01, mu=1e-8, eps=1e-5, max_iter=100, visualize=False):
    """
    Performs Hypergradient Descent optimization on the given function.
    ...
    while np.linalg.norm(g) > eps and i < max_iter:
        ...
        # Store previous gradient and update current gradient
        g_prev = np.copy(g)
        g = nabla_f(x)
    ...
    if visualize:
        ...
        plt.ylabel("x[1]")
        plt.legend()
        plt.title("Real-time Hypergradient Descent Optimization Path")
        plt.show()
...
# Initial guess
x0 = np.array([5, 5])
x_optimal = hyper_gradient_descent(f, nabla_f, x0, visualize=True)
